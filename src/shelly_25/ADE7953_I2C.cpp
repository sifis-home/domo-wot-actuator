...
uint8_t ADE7953::getVersion() {
    return i2cAlgorithm8_read(functionBitVal(Version_8, 1), functionBitVal(Version_8,
                                                                           0));  //An example of the address lookup - the spiAlgorithm8_read((functionBitVal(addr,1), functionBitVal(addr,1)) would return the eqivenet to spiAlgorithm8_read(0x07,0x02) when working properly
}
...
unsigned long ADE7953::getAPNOLOAD() {  //use signed long for signed registers, and unsigned long for unsigned registers
    ...
    value = i2cAlgorithm32_read((functionBitVal(AP_NOLOAD_32, 1)), (functionBitVal(AP_NOLOAD_32,
                                                                                   0))); //Call MSB and LSB from the register constant (template for how all functions should be called)
    return value;
}
...
float ADE7953::getVrms() {
    ...
    float decimal = decimalize(value, 26151, 0);
    return decimal;
}
...
float ADE7953::getActiveEnergyA() {
    long value = 0;
...
float ADE7953::getApparentEnergyA() {
    long value = 0;
...
float ADE7953::getInstReactivePowerB() {
    long value = 0;
    value = i2cAlgorithm32_read((functionBitVal(BVAR_32, 1)), (functionBitVal(BVAR_32, 0)));
    float decimal = decimalize(value, 1.502, 0);
    return decimal;
}
...
//    delay(50);
    ...
    i2cAlgorithm16_write((functionBitVal(PHCALA_16, 1)), (functionBitVal(PHCALA_16, 0)), 0x00,
                         0x00);  //This may need to be turned off if there are issues!
    delay(100);
    i2cAlgorithm16_write((functionBitVal(PHCALB_16, 1)), (functionBitVal(PHCALB_16, 0)), 0x00,
                         0x00);  //This may need to be turned off if there are issues!
    delay(100);

    i2cAlgorithm32_write((functionBitVal(AP_NOLOAD_32, 1)), (functionBitVal(AP_NOLOAD_32, 0)), 0x00, 0x00, 0x00,
                         0x01); //Check for ensuring read and write operations are okay
    delay(100);
    i2cAlgorithm8_write((functionBitVal(LCYCMODE_8, 1)), (functionBitVal(LCYCMODE_8, 0)),
                        0b01111111); //Enable line cycle accumulation mode for all energies and channels
    delay(100);
    i2cAlgorithm16_write((functionBitVal(LINECYC_16, 1)), (functionBitVal(LINECYC_16, 0)), 0x00,
                         0x78); //Sets number of half line cycle accumulations to 120
    delay(100);
...
uint8_t ADE7953::i2cAlgorithm8_read(byte MSB,
...
uint16_t ADE7953::i2cAlgorithm16_read(byte MSB,
                                      byte LSB) { //This is the algorithm that reads from a register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.
#ifdef ADE7953_VERBOSE_DEBUG
...
uint32_t ADE7953::i2cAlgorithm24_read(byte MSB,
                                      byte LSB) { //This is the algorithm that reads from a register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.
#ifdef ADE7953_VERBOSE_DEBUG
    ...
    //Post-read packing and bitshifting operation
    readval_unsigned = (((uint32_t) one << 16) + ((uint32_t) two << 8) +
                        ((uint32_t) three)); //(Alternative shift algorithm)
...
uint32_t ADE7953::i2cAlgorithm32_read(byte MSB,
                                      byte LSB) { //This is the algorithm that reads from a 32 bit register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.  Caution, some register elements contain information that is only 24 bit with padding on the MSB
#ifdef ADE7953_VERBOSE_DEBUG
...
void ADE7953::i2cAlgorithm32_write(byte MSB, byte LSB, byte onemsb, byte two, byte three,
                                   byte fourlsb) { //This is the algorithm that writes to a register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.
#ifdef ADE7953_VERBOSE_DEBUG
...
void ADE7953::i2cAlgorithm24_write(byte MSB, byte LSB, byte onemsb, byte two,
                                   byte threelsb) { //This is the algorithm that writes to a register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.
#ifdef ADE7953_VERBOSE_DEBUG
...
void ADE7953::i2cAlgorithm16_write(byte MSB, byte LSB, byte onemsb,
                                   byte twolsb) { //This is the algorithm that writes to a register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.
#ifdef ADE7953_VERBOSE_DEBUG
...
void ADE7953::i2cAlgorithm8_write(byte MSB, byte LSB,
                                  byte onemsb) { //This is the algorithm that writes to a register in the ADE7953. The arguments are the MSB and LSB of the address of the register respectively. The values of the arguments are obtained from the list of functions above.
#ifdef ADE7953_VERBOSE_DEBUG
...
float ADE7953::decimalize(long input, float factor,
                          float offset) //This function adds a decimal point to the input value and returns it as a float
{
#ifdef ADE7953_VERBOSE_DEBUG
...
float ADE7953::getActiveEnergyB() {
    long value = i2cAlgorithm32_read((functionBitVal(AENERGYB_32, 1)), (functionBitVal(AENERGYB_32, 0)));
    float decimal = decimalize(value, 1.502, 0);
    return decimal;
}

float ADE7953::getInstActivePowerB() {
    long value = i2cAlgorithm32_read((functionBitVal(BWATT_32, 1)), (functionBitVal(BWATT_32, 0)));
    float decimal = decimalize(value, 1.502, 0);
    return abs(decimal);
}


float ADE7953::getApparentEnergyB() {
    long value = 0;
    value = i2cAlgorithm32_read((functionBitVal(APENERGYB_32, 1)), (functionBitVal(APENERGYB_32, 0)));
    float decimal = decimalize(value, 1.502, 0);
    return decimal;
}